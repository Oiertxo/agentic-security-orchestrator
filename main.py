    ...
    async def event_generator():
        try:
            ...
            if security_graph is None:
                logger.error(
                    "[MAIN] Security graph was not initialized during lifespan."
                )
                ...
                if snapshot.next:
                    if "approve" in query:
                        ...
                        await security_graph.aupdate_state(
                            config, {"next_step": "report"}
                        )
                        async for chunk in run_graph_stream(None, config, thread_id):
                            yield chunk
                    else:
                        async for chunk in handle_consultation_stream(
                            request.query, snapshot.values, thread_id, False
                        ):
                            yield chunk
                else:
                    # Post-Audit Analysis
                    async for chunk in handle_consultation_stream(
                        request.query, snapshot.values, thread_id, True
                    ):
                        yield chunk
            else:
                # Default: new thread
...
async def get_detailed_status(thread_id: str):
    ...
    """
    if security_graph is None:
        logger.error("[MAIN] Security graph not initialized")
    ...
    try:
        ...
        exploit_count = (
            sum(len(e) for e in exploit_data.values()) if exploit_data else 0
        )
        ...
        raise HTTPException(
            status_code=500, detail="Internal error retrieving thread state"
        )
...
async def fork_audit(source_thread_id: str, new_thread_id: str):
    ...
    try:
        ...
        await security_graph.aupdate_state(
            new_config, snapshot.values, as_node="supervisor"
        )
        ...
        logger.error(
            f"[MAIN] Error forking thread {source_thread_id} to {new_thread_id}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="Internal error during thread forking"
        )
...
def check_deployment():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(
                f"OLLAMA: Service answers but with error {response.status_code}."
            )
    except Exception:
        logger.error("OLLAMA: Unreachable. Check if Ollama is running.")
        status = False
    ...
    try:
        ...
        logger.info("ENVIRONMENT: LangGraph and dependencies correctly installed.")
    except ImportError:
...
async def format_graph_response(result, config, thread_id):
    ...
    return {"response": last_message, "status": "success", "thread_id": thread_id}
