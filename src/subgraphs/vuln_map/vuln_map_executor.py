...
def vuln_map_executor_node(state: AgentState) -> AgentState:
    ...
    new_analyzed_services_for_search = {ip: list(ports) for ip, ports in analyzed_services_for_search.items()}
    ...
    try:
        ...
        return {
            **state,
            "messages": [HumanMessage(content=f"[SOURCE: vuln_map_engine]\n{json.dumps(result)}")],
            "vuln_map": {**old_vuln_map, "results": (old_vuln_map.get("results", [])) + [result], "step_count": new_step},
            "next_step": "planner"
        }
    ...
    if engine_result.get("ok"):
        ...
        raw_exploits.sort(key=lambda x: (x.get("Verified") == "1", x.get("EDB-ID")), reverse=True)
        ...
        existing_ids = {e.get("edb_id") for e in existing_exploits if isinstance(e, dict)}

        new_to_add = [n for n in new_found_exploits if n.get("edb_id") not in existing_ids]
